...
The Line/Branch coverage badges at the top of this README are regenerated by the [CI pipeline](https://github.com/GVLTodorov/Monorepo/actions/workflows/ci.yml) on every push to `main`, covering all three libraries combined. For a per-library breakdown, see that pipeline's job summary or download its `coverage-report` artifact for the full HTML report.
