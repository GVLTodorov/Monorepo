...
[TestFixture]
public sealed class SqliteRepositoryMutationTests
{
    ...
    [Test]
    public async Task TruncateTableAsync_DeletesAllRowsIncludingSoftDeleted()
    {
        var commands = new CommandCaptureInterceptor();
        await using var database = await CreateSeededDatabaseAsync(commands);
        var repository = new SqliteRepository<TestRecord>(database.Context);
        ...
        {
            Assert.That(await database.Context.Entities.CountAsync(), Is.Zero);
            Assert.That(commands.Commands, Has.Some.EqualTo("DELETE FROM \"test_records\";"));
        });
    }

    [Test]
    public async Task TruncateTableAsync_RestartIdentityWithoutSequenceTable_SkipsSequenceCleanup()
    {
        var commands = new CommandCaptureInterceptor();
        await using var database = await CreateSeededDatabaseAsync(commands);
        var repository = new SqliteRepository<TestRecord>(database.Context);
        ...
        {
            Assert.That(commands.Commands, Has.Some.EqualTo("DELETE FROM \"test_records\";"));
            Assert.That(commands.Commands, Has.None.Contains("sqlite_sequence"));
        });
    }

    [Test]
    public async Task TruncateTableAsync_RestartIdentityWithSequenceTable_ResetsAutoincrementCounter()
    {
        var commands = new CommandCaptureInterceptor();
        await using var database = await CreateSeededDatabaseAsync(commands);
        ...
        {
            command.CommandText = """
                CREATE TABLE autoincrement_probe (id INTEGER PRIMARY KEY AUTOINCREMENT, value TEXT);
                INSERT INTO autoincrement_probe (value) VALUES ('probe');
                """;
        ...
        {
            Assert.That(commands.Commands, Has.Some.EqualTo("DELETE FROM \"test_records\";"));
            Assert.That(commands.Commands, Has.Some.EqualTo("DELETE FROM sqlite_sequence WHERE name = 'test_records';"));
        });
    }
    ...
    {
        var database = await TestDatabase.CreateAsync(createEntityTable: true, interceptors);
