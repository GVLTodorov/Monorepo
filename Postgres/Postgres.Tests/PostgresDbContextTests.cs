...
public sealed class PostgresDbContextTests
{
    ...
    {
        Assert.That(
            () => new PostgresDbContext<TestRecord>(connectionString!),
            Throws.InstanceOf<ArgumentException>());
    }

    [Test]
    public void ConnectionStringConstructor_ValidString_ConfiguresNpgsqlWithoutConnecting()
    {
        using var context = new PostgresDbContext<TestRecord>(
    ...
    {
        using var context = new PostgresDbContext<TestRecord>(
            new DbContextOptionsBuilder<PostgresDbContext<TestRecord>>().Options);
