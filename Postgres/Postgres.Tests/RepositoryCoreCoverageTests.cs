...
internal sealed class CoverageRecord : PostgresEntity
{
...
internal sealed class CoverageRepository(DbConnection connection)
    : PostgresRepository<CoverageRecord>(connection)
{
    public bool UsesProvider => UsesNpgsqlProvider;
}

[TestFixture]
public sealed class RepositoryCoreCoverageTests
{
    private const string RepositoryNamespace = "Postgres";
    private static readonly Assembly RepositoryAssembly = typeof(PostgresRepository<>).Assembly;

    [SetUp]
    public void InitializeSqliteProvider() => SqliteTestProvider.EnsureInitialized();
    ...
    public async Task PublicRepository_CoversValidationEmptyAndMultiplicityBranches()
    {
        ...
        Assert.That(repository.UsesProvider, Is.False);
        await using var owned = new PostgresRepository<CoverageRecord>(
            "Host=localhost;Database=coverage;Username=coverage;Password=coverage");
    ...
    private static object CreateCore(DbConnection connection, string dialectName, string tableName = "CoverageRecords")
    {
        ...
        var coreType = RepositoryAssembly
            .GetType($"{RepositoryNamespace}.PostgresRepositoryCore`1", throwOnError: true)!
    ...
    private static Expression InvokeRepositoryStripConvert(Expression expression) =>
        (Expression)typeof(PostgresRepository<CoverageRecord>)
